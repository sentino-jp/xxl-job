...
**部署拓扑**：调度中心 2 实例共享一个 PostgreSQL（本分支已从 MySQL 移植），靠 xxl_job_lock 表互斥；服务端口 9280，前面由 Internal Gateway 以内网域名 xxl-job.internal.sentino.jp 反代。执行器按业务域分组，每组至少 2 实例。调度中心与执行器之间用执行器维度的 AccessToken。
