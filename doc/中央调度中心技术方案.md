...
JobAlarm 接口现有两个实现，任务失败时由 JobAlarmer 逐个调用，全部成功才把日志标记为"告警成功"：

- **EmailJobAlarm**：上游自带，按任务的报警邮箱字段发邮件。
- **LarkJobAlarm**：本分支新增，飞书 / Lark 自定义机器人 webhook，实现参考 coucou-server 的 FeishuAlertNotifier。配置项 `xxl.job.alarm.lark.webhook-url`、`secret`、`env`、`suppress-window-seconds`、`admin-url`，值走环境变量；未配置 webhook 时为空操作且返回成功，不影响邮件通道。

Lark 通道的三个要点：飞书失败也返回 HTTP 200，必须看响应体 code 是否为 0；单机器人限流每分钟 100 次，因此按任务做抑制窗口，同一任务在窗口内只发一条，其余计数并在下次放行时说明；消息前缀带环境标识，stage 与 prod 发同一个群时可区分。消息内容含任务、执行器、JobHandler、执行地址、调度时间、调度与执行结果（已剥掉 HTML 标签）、日志 ID 以及执行日志链接。
...
**部署拓扑**：调度中心部署在 coucou-server 同一批三台 OCI 节点（10.0.0.100 / 10.0.0.194 / 10.0.0.228），共享一个 PostgreSQL（本分支已从 MySQL 移植），靠 xxl_job_lock 表互斥；服务端口 8088，前面由 Internal Gateway 以内网域名 xxl-job.internal.sentino.jp 反代。执行器按业务域分组，每组至少 2 实例。调度中心与执行器之间用执行器维度的 AccessToken。

### 8.2 执行器

- **内置 HTTP Handler**：xxl-job-core 自带 httpJobHandler，每个执行器启动时自动注册，可用配置关闭。控制台新建任务时"调用方式"选 HTTP，填 URL、Method、Headers、Body、超时即可，参数会序列化成 JSON 存进任务参数；业务服务只需暴露一个接口，不必写 Java 代码。目标域名白名单在执行器侧配置（httpjob.allowdomains），Header 里的凭证可写 `${ENV_NAME}` 在执行器侧用环境变量解析，调度中心不持有凭证。
- **通用 HTTP 执行器**：内嵌在调度中心（xxl-job-admin）进程里，只带内置 HTTP Handler，供 coucou-server 这类不嵌入 xxl-job-core 的服务使用；每台调度中心节点即一个实例，AppName 为 http-executor，可用 XXL_JOB_EXECUTOR_ENABLED=false 关闭某台。（早期的独立模块 xxl-job-executor-http 已于 2026-09-16 并入调度中心。）
...
- 飞书 JobAlarm 已实现（LarkJobAlarm），上线时配置 webhook 与环境标识。
