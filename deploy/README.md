...
XXL_JOB_HTTPJOB_ALLOWDOMAINS=.sentino.jp,.coucou.fun,.changeeon.com.cn
```

`XXL_JOB_HTTPJOB_ALLOWDOMAINS` 生产只放行 `sentino.jp` / `coucou.fun` / `changeeon.com.cn` 三个域名入口（点前缀写法 = 顶级域本身 + 全部子域），这也是 application.properties 的内置默认值；**不要显式留空**，空值 = 允许任意地址，内嵌执行器会变成内网 SSRF 跳板。要调别的第三方接口时加精确主机名或 URL 前缀，不要放开。`XXL_JOB_EXECUTOR_ACCESS_TOKEN` 首次上线时执行器组还没建，先留空启动：此时内嵌执行器**不启动**，日志有一条 `embedded http executor NOT started: accessToken empty` 的 WARN，只影响 HTTP 任务，不影响调度；第 5 步生成 token 后回填并滚动重启，执行器随之启动。
