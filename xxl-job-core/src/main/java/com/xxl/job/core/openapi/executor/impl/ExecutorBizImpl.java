...
import com.xxl.job.core.handler.impl.HiveJobHandler;
...
public class ExecutorBizImpl implements ExecutorBiz {
    ...
    public Response<String> trigger(TriggerRequest triggerRequest) {
        ...
        if (GlueTypeEnum.BEAN == glueTypeEnum) {
        ...
        } else if (GlueTypeEnum.GLUE_HIVESQL == glueTypeEnum) {

            // valid old jobThread
            if (jobThread != null &&
                    !(jobThread.getHandler() instanceof HiveJobHandler
                            && ((HiveJobHandler) jobThread.getHandler()).getGlueUpdatetime()== triggerRequest.getGlueUpdatetime() )) {
                // change sql or gluesource updated, need kill old thread
                removeOldReason = "change job source or glue type, and terminate the old job thread.";
            ...
            // valid handler
            if (jobHandler == null) {
                jobHandler = new HiveJobHandler(triggerRequest.getJobId(), triggerRequest.getGlueUpdatetime(), triggerRequest.getGlueSource());
            }
